...
class Service:
    ...
    async def play(self, body):
        ...
        candidate = None
        try:
            resolved = await self.resolver.resolve(body["kind"], body["id"], body.get("season", 1), body.get("episode", 1))
            self.job_state.update(title=resolved["title"], resolve_seconds=round(time.monotonic() - started, 3))
            failures = []
            for source in resolved["sources"]:
                self.job_state["state"] = "preparing"
                candidate = Session(self.fetcher, self.config, source, resolved["headers"], resolved["title"])
                try:
                    ...
                    await self.roku.launch(candidate.link(candidate.root), candidate.title)
                    ...
                    if self.session is candidate:
                        self.session = None
                    candidate = None
            raise ValueError("sources failed: " + "; ".join(failures))
    ...
    async def start_play(self, request):
        ...
        if current["state"] == "play" and current["app_id"] != "562859" and body.get("replace") is not True:
            raise web.HTTPConflict(text="TV is playing; explicit replace=true required")
        if "kind" not in body or "id" not in body:
    ...
    async def status(self, request):
        ...
        if self.session:
            result["relay"] = {**self.session.metrics, "cache_bytes": self.session.cache_size}
    ...
    async def media(self, request):
        ...
        if (session is None or request.match_info["token"] != session.token
                or session.closed):
        ...
        if value := request.headers.get("Range"):
            ...
            return web.Response(body=data[start:end + 1], status=206, content_type=content_type, headers=headers)
...
async def serve(config):
    ...
    control.add_routes([web.get("/status", service.status), web.get("/search", service.search),
                        web.post("/play", service.start_play), web.post("/command/{command}", service.command)])
    media = web.Application(middlewares=[errors], client_max_size=1024)
    media.add_routes([web.get("/media/{token}/{key}", service.media)])
    runners = [web.AppRunner(control, access_log=None), web.AppRunner(media, access_log=None)]
    ...
    try:
        ...
        logging.info("roomcast ready; control=%s media=loopback:%s", socket, config.media_port)
